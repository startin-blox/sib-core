...
const SIBListMixin = superclass => class extends superclass {
  ...
  applyFilterToResource(resource, filter) {
    ...
    const firstFilter = filter.shift();
  ...
  matchFilters(resource) {
    ...
    return Object.keys(this.filters).reduce(
      (initial, filter) => initial && this.matchFilter(resource, filter, this.filters[filter]),
      true,
    );
  }
  ...
  get paginateBy() {
    ...
    if (Number.isNaN(pagination)) return null;
    return pagination;
  }
  ...
  appendFilters() {
    ...
    formElt.fields.forEach((field) => {
      ['range', 'widget', 'label'].forEach((attr) => {
    ...
    if (this.shadowRoot) this.shadowRoot.insertBefore(formElt, this.shadowRoot.firstChild);
  ...
  populate() {
    ...
    this.currentPageResources.forEach((resource) => {
      ...
      if (resource['@type'] !== 'sib:source') {
        ...
        store.get(resource.container).then((container) => {
          container['ldp:contains'].forEach((res) => {
            this.appendChildElt(res);
          });
        });
      }
    });
