export class SparqlQueryFactory {
    ...
    public static makeSkillAndCityIndexQuery(skills: string, city: string): string {
        return `PREFIX idx: <https://ns.inria.fr/idx/terms#>
        PREFIX sib: <http://cdn.startinblox.com/owl/ttl/vocab.ttl#>
        PREFIX sh: <https://www.w3.org/ns/shacl#>
        SELECT ?result 
    ...
    public static makeSkillOrCityAndNameIndexQuery(skillOrCityPredicate: string, skillsOrCities: string, patterns: string, namePath: string): string {
        return `PREFIX idx: <https://ns.inria.fr/idx/terms#>
        PREFIX sib: <http://cdn.startinblox.com/owl/ttl/vocab.ttl#>
        PREFIX sh: <https://www.w3.org/ns/shacl#>
        SELECT ?result 
    ...
    public static makeSkillAndCityAndNameIndexQuery(skills: string, city: string, patterns: string, namePath: string): string {
        return `PREFIX idx: <https://ns.inria.fr/idx/terms#>
        PREFIX sib: <http://cdn.startinblox.com/owl/ttl/vocab.ttl#>
        PREFIX sh: <https://www.w3.org/ns/shacl#>
        SELECT ?result 
    ...
    public static makeIndexSkillCityQuery(skills: string[], city: string): string {
        return SparqlQueryFactory.makeSkillAndCityIndexQuery(skills.map(s => `<${s}>`).join(', '), `"${city.toLowerCase()}"`);
    }

    public static makeIndexSkillFirstNameQuery(skills: string[], patterns: string[]): string {
    ...
    public static makeIndexSkillLastNameQuery(skills: string[], patterns: string[]): string {
        return SparqlQueryFactory.makeSkillOrCityAndNameIndexQuery(
            "sib:skill", 
            skills.map(s => `<${s}>`).join(', '), 
            patterns.map(p => `"${p}.*"`).join(', '),
    ...
    public static makeIndexCityFirstNameQuery(city: string, patterns: string[]): string {
        return SparqlQueryFactory.makeSkillOrCityAndNameIndexQuery(
            "sib:city", 
    ...
    public static makeIndexCityLastNameQuery(city: string, patterns: string[]): string {
        return SparqlQueryFactory.makeSkillOrCityAndNameIndexQuery(
            "sib:city", 
            `"${city.toLocaleLowerCase()}"`, 
            patterns.map(p => `"${p}.*"`).join(', '),
    ...
    public static makeIndexSkillAndCityFirstNameQuery(skills: string[], city: string, patterns: string[]): string {
    ...
    public static makeIndexSkillAndCityLastNameQuery(skills: string[], city: string, patterns: string[]): string {
        return SparqlQueryFactory.makeSkillAndCityAndNameIndexQuery(
            skills.map(s => `<${s}>`).join(', '), 
            city, 
            patterns.map(p => `"${p}.*"`).join(', '),
