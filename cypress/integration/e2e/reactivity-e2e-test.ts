...
describe('Reactivity e2e test', function () {
  ...
  it('check display', () => {
    ...
    cy.get('solid-display#user > solid-display-value:nth-child(1)').should('have.attr', 'name', 'first_name').should('contain', 'Matthieu');
    cy.get('solid-display#user > solid-display-value:nth-child(2)').should('have.attr', 'name', 'last_name').should('contain', 'Fesselier');
    cy.get('solid-display#user > solid-display-value:nth-child(3)').should('have.attr', 'name', 'username').should('contain', 'matthieu');
    cy.get('solid-display#user > solid-display-value:nth-child(4)').should('have.attr', 'name', 'profile.city').should('contain', 'Rennes');
    cy.get('solid-display#profile > solid-display-value:nth-child(1)').should('have.attr', 'name', 'city').should('contain', 'Rennes');
    cy.get('solid-display#circle > solid-display-value:nth-child(1)').should('have.attr', 'name', 'owner.profile.city').should('contain', 'Rennes');
    cy.get('solid-display#profile-widget > custom-widget').should('have.attr', 'name', 'profile');
    cy.get('solid-display#profile-widget > custom-widget > div').should('contain', 'Rennes');
  ...
  it('has reactive nested resources', () => {
    ...
    cy.get('solid-display#user > solid-display-value[name="profile.city"]').should('contain', 'Paris');

    // Nested resource in nested field
    cy.get('solid-display#profile > solid-display-value[name="city"]').should('contain', 'Paris');

    // Nested resource in custom widget
    cy.get('solid-display#profile-widget > custom-widget[name="profile"] > div').should('contain', 'Paris');
  ...
  it('has reactive properties', () => {
    ...
    // Single sib-display
    cy.get('solid-display#user > solid-display-value[name="first_name"]').should('contain', 'Test');
    cy.get('solid-display#user > solid-display-value[name="last_name"]').should('contain', 'User');
    cy.get('solid-display#user > solid-display-value[name="username"]').should('contain', 'admin');

    // List sib-display & range
    const src = "https://ldp-server.test/users/matthieu/";
    cy.get(`solid-display#users solid-display[data-src="${src}"] > solid-display-value[name="first_name"]`).should('contain', 'Test');
    cy.get(`solid-display#users solid-display[data-src="${src}"] > solid-display-value[name="last_name"]`).should('contain', 'User');
    cy.get(`solid-display#users solid-display[data-src="${src}"] > solid-display-value[name="username"]`).should('contain', 'admin');
    cy.get(`solid-form#range option[value='{"@id": "${src}"}']`).should('contain', 'Test User');

    // Federation
    cy.get(`solid-display#federation solid-display[data-src="${src}"] > solid-display-value[name="first_name"]`).should('contain', 'Test');
    cy.get(`solid-display#federation solid-display[data-src="${src}"] > solid-display-value[name="last_name"]`).should('contain', 'User');
    cy.get(`solid-display#federation solid-display[data-src="${src}"] > solid-display-value[name="username"]`).should('contain', 'admin');
    ...
    cy.get(`solid-display#users solid-display[data-src="${newSrc}"] > solid-display-value[name="first_name"]`).should('contain', 'Alex');
    cy.get(`solid-display#users solid-display[data-src="${newSrc}"] > solid-display-value[name="last_name"]`).should('contain', 'Bourlier');
    cy.get(`solid-display#users solid-display[data-src="${newSrc}"] > solid-display-value[name="username"]`).should('contain', 'alex');
    cy.get(`solid-form#range option`).should('have.length', 4);

    // Federation
    cy.get(`solid-display#federation solid-display[data-src="${newSrc}"] > solid-display-value[name="first_name"]`).should('contain', 'Alex');
    cy.get(`solid-display#federation solid-display[data-src="${newSrc}"] > solid-display-value[name="last_name"]`).should('contain', 'Bourlier');
    cy.get(`solid-display#federation solid-display[data-src="${newSrc}"] > solid-display-value[name="username"]`).should('contain', 'alex');
